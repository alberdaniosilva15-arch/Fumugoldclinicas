// ═══════════════════════════════════════════════════════════
// FUMUGOLD LOCAL TOOLS v2.0
// Ferramentas locais: arquivo, CSV, IA local, download
// ═══════════════════════════════════════════════════════════
...
export function buildBundleCSV(bundle) {
  ...
  Object.entries(meta.totals || {}).forEach(([k, v]) => {
    lines.push(`${k},${v}`);
  });
  lines.push('');
...
export async function persistArchiveBundle(bundle, opts = {}) {
  ...
  try {
    ...
    // Download automático se writeToFolder=true (browser não tem acesso a pasta)
    if (writeToFolder) {
    ...
    return {
      ...
      folderError:  writeToFolder
        ? 'Browser não suporta acesso directo a pastas. Ficheiro descarregado para Downloads.'
        : '',
    };
  } catch (e) {
    return {
...
export function buildLocalAIResponse(prompt = '', bundle = {}) {
  ...
  const response = [
    ...
    '',
    '─────────────────────────────────',
    '⚡ Análise gerada localmente — sem rede.',
  ].join('\n');
...
function _analyseClinicData(bundle) {
  ...
  const consHoje = appointments.filter(a =>
    (a.data || '').slice(0, 10) === hoje
  );
  ...
  if (camasTotal > 0) {
    ...
    if (taxa > 85) alerts.push(`LOTAÇÃO: ${taxa}% de ocupação — considerar transferências`);
  }

  // Notificações não lidas
  const naoLidas = notifications.filter(n => !n.lida && !n.read).length;
  if (naoLidas > 0) {
    alerts.push(`${naoLidas} notificação(ões) por ler`);
  }
